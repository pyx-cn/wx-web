    ...
    <div class="wrapper">
        ...
        <ul>
            <li v-for="(item, index) in list.data" :key="index">
                ...
                <div class="right" v-if="item.orders.state === 1" @click="toCancel(index)">
                    取消预约
                </div>
                <div class="right" v-if="item.orders.state === 0" @click="toApp(index)">
                    重新预约
                </div>
                <div class="right" v-if="item.orders.state === 3">
                    已归档
                </div>
            </li>
        </ul>
...
function getOrdersByUserId() {
    ...
    axios.get('http://localhost:8080/getOrdersByUserId', {
        params: {
            userId: user.userid
        },
        headers: {
...
const toCancel = (index) => {
    ...
    axios.post('http://localhost:8080/cancelOrder', `orderId=${item.orders.orderid}`, {
        headers: {
            'Authorization': `${token}`
        }
    })
...
const toApp = (index) => {
    ...
    axios.post('http://localhost:8080/reOrder', `orderId=${item.orders.orderid}`, {
        headers: {
            'Authorization': `${token}`
        }
    })
