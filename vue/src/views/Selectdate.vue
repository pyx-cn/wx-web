            ...
            for (let i = 0; i < calendarArr.value.length; i++) {
                ...
                if (calendarArr.value[i].ymd != null) {
                    ...
                    calendarArr.value[i].day = parseInt(
                        calendarArr.value[i].ymd.substring(8)
                    );
...
function selectDay(index) {
    ...
    selectDay = calendarArr.value[index].ymd;
}
...
function toConfirmOrder() {
    ...
    router.push({ path: '/confirmOrder', query: { hpId: hpId, smId: smId, selectDay: selectDay } });
}
