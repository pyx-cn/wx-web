...
import { ref, reactive, toRefs, onMounted, toDisplayString, onRenderTriggered } from "vue";
...
const getHospital = async () => {
    ...
    await axios.get('http://localhost:8080/getHospitalById', {
        params: {
            hpId: hpId
        }, headers: {
            'Authorization': `${token}`
...
const getSetmealById = async () => {
    const token = getSessionStorage("token");
    await axios.get('http://localhost:8080/getSetmealById', {
        params: {
            smId: smId
        },
        headers: {
